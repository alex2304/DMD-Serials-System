...
def process_actors(actor_id=None):
    ...
    if actor_id:
        return process_person_page(actor_id, PersonsTypes.ACTOR)
...
def process_writers(writer_id = None):
    if writer_id:
        return process_person_page(writer_id, PersonsTypes.WRITER)

    person_name_part = request.args.get('person_name_part')
    if person_name_part:
        filtered_persons = PersonsRepository.get_filtered_persons(person_name_part, PersonsTypes.WRITER)
        return Response(json.dumps(filtered_persons), mimetype='application/json')
...
def process_directors(director_id = None):
    if director_id:
        return process_person_page(director_id, PersonsTypes.DIRECTOR)

    person_name_part = request.args.get('person_name_part')
    if person_name_part:
        filtered_persons = PersonsRepository.get_filtered_persons(person_name_part, PersonsTypes.DIRECTOR)
        return Response(json.dumps(filtered_persons), mimetype='application/json')

    return render_template('directors.html')


def process_person_page(person_id, person_type):
    ...
    if person_info:
        return render_template('person_info.html', person_info=person_info, episodes_info=episodes_info)
    else:
        abort(404)